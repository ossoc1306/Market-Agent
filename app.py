...
with col_left:
    with st.expander("🔍 Momentum & Trend Layers (SPX, QQQ, VXUS)", expanded=True):
        ...
        with c1:
            ...
            st.write(f"Price: {spx_now:,.2f} | RSI-D: {spx_rsi_d:.1f} | Weekly: {spx_rsi_w:.1f}")
        with c2:
            st.write("**Nasdaq (QQQ)**")
            st.write(f"Price: {qqq_now:,.2f} | RSI-D: {qqq_rsi_d:.1f} | Weekly: {qqq_rsi_w:.1f}")
        with c3:
            st.write("**International (VXUS)**")
            st.write(f"Price: {vxus_now:,.2f} | RSI-D: {vxus_rsi_d:.1f} | Weekly: {vxus_rsi_w:.1f}")
