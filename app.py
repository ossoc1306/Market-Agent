...
with col_left:
    with st.expander("🌍 Geopolitical Risk Agent", expanded=True):
        st.write("**Headline: Techno-Nationalism & Great Power Friction (2026)**")
        st.write("Current Focus: US-China chip decoupling and Western Hemisphere intervention strategy.")
        st.write("---")
        st.write("**Impacted Assets:**")
        st.write("🟢 **Bullish:** Gold (Safe Haven), Defense Stocks ( Rearmament), Cybersecurity (Gray Zone War)")
        st.write("🔴 **Bearish:** Emerging Markets (Decoupling), Consumer Tech (Supply Shocks), Global Bonds (Inflation Risk)")
        st.info("Agent Logic: In a 'Polycrisis' environment, gold independent of yields becomes the primary liquidity anchor.")
    ...
    with st.expander("🔍 Momentum & Trend Layers (SPX & QQQ)", expanded=True):
        ...
        st.info("Trend Note: Primary trend remains UP while holding the 40-week line.")
...
with col_right:
    with st.expander("📊 Inflation & Growth Dynamics", expanded=True):
        st.write("**Core PCE:** 3.0% YoY | **GDP:** 1.4% (Q4)")
        st.warning("Analysis: Watch for 'Stagflation' signals if growth cools while PCE stays sticky.")
    ...
    with st.expander("🏦 Yield Curve & Interest Rates", expanded=True):
        st.write(f"**10Y Benchmark:** {tnx_now:.2f}% | **3M Bill:** {short_rate:.2f}%")
    ...
    with st.expander("📜 Fiscal Policy & Treasury Issuance", expanded=True):
        ...
        st.info("Strategy: A drop in 'Bill Share' relative to Coupons usually precedes volatility spikes.")
