...
with col_left:
    with st.expander("🔍 Momentum & Trend Layers (SPX, QQQ, VXUS)", expanded=True):
        ...
        with c1:
            ...
            st.write(f"Price: {spx_now:,.2f} | RSI: {spx_rsi_d:.1f}")
        with c2:
            st.write("**Nasdaq (QQQ)**")
            st.write(f"Price: {qqq_now:,.2f} | RSI: {calculate_rsi('QQQ'):.1f}")
        with c3:
            st.write("**International (VXUS)**")
            st.write(f"Price: {vxus_now:,.2f} | RSI: {vxus_rsi_d:.1f}")
    ...
    with st.expander("📊 Inflation & Growth Dynamics", expanded=True):
        st.write("**Core PCE:** 3.0% YoY | **GDP Growth:** 1.4%")
...
with col_right:
    with st.expander("🌊 Liquidity Watch Agent (US & Global)", expanded=True):
        ...
        st.write("**Global M2 Proxy:** Tracking the $99.8T global aggregate.")
        st.info("**Global Analysis:** Rising Global M2 creates a 'liquidity tailwind' for risk assets. However, a spiking DXY acts as a vacuum that offsets M2 expansion.")
        st.write("**US Net Liquidity Logic:** Fed Balance Sheet - (TGA + RRP)")

    with st.expander("✨ Gold Intelligence Agent", expanded=True):
        st.write(f"**Gold:** ${gold_now:,.2f} | Ratio: {gold_now/spx_now if spx_now > 0 else 0:.4f}")

    with st.expander("🏦 Yield Curve & Interest Rates", expanded=True):
        st.write(f"**10Y Yield:** {tnx_now:.2f}% | **10Y/3M Spread:** {tnx_now - short_rate:.2f}%")

    with st.expander("📜 Fiscal Policy & Treasury Issuance", expanded=True):
        st.write("**Recent QRA:** Treasury offering $125B (Feb 2026).")
        st.info("**Analysis:** A shift from T-Bills to Coupons drains bank reserves. This usually increases equity volatility.")
...
with geo_left:
    ...
    st.write("- **Trade Friction:** Tariffs and export controls on semiconductors.")
    st.write("- **Energy Stability:** Maritime corridor tensions impacting oil costs.")
    st.write("- **Asset Impact:** 🔴 Bearish for Emerging Markets and Consumer Tech.")
with geo_right:
    st.write("**🟢 Geopolitical Tailwinds**")
    st.write("- **Near-Shoring:** Industrial CapEx in Western Hemisphere.")
    st.write("- **Defense Modernization:** Multi-year budget expansion.")
    st.write("- **Asset Impact:** 🟢 Bullish for Defense, Cybersecurity, and Gold.")

st.caption(f"Update: {datetime.now().strftime('%Y-%m-%d %H:%M')} | Data Source: [Yahoo Finance](https://finance.yahoo.com)")
